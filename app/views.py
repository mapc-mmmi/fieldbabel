...
def index():
    ...
    current_form = CurrentDataForm(prefix="current_form")
    crop_form = CropSelectorForm(prefix="crop_form")
    if current_form.submit_current.data and current_form.validate_on_submit():
        if(valid_email(current_form.email.data) and is_json(current_form.fieldpoly.data)):
            if(valid_field(current_form.fieldpoly.data)):
                ...
                flash('Data is going to be send to: %s' % current_form.email.data)
            ...
            if(is_json(current_form.fieldpoly.data)):
                flash('Not a valid email:%s' % current_form.email.data)
            else:
                flash('BROKEN JSON SYNTAX! Did you copy it correctly')
        return redirect('/')

    elif crop_form.crops_submit.data and crop_form.validate_on_submit():
        if(valid_email(crop_form.email.data) and is_json(crop_form.fieldpoly.data)):
            if(valid_field(crop_form.fieldpoly.data)):
                ...
                rq = models.Request(body=crop_form.fieldpoly.data, timestamp=datetime.datetime.utcnow(), crop_type=crop_form.crop_type.data, season = crop_form.season.data, processed=False, author=u, request_queued=False) #crop_type="Winterwheat"
                db.session.add(rq)
                db.session.commit()
